...
namespace BlueChappie
{
    class clsMainProgram
    {
        ...
        public string _ClientRequestId = "";

        //
        //  doSyncWork and bw_DoWork- Thank you microsoft. This takes care of my background online scan for images.
        //
       
        public void doSyncWork(String Location,String ClientRequestId)
        {
            _location = Location;
        ...
        private void bw_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            SyncImages(_ClientRequestId,_location);
        }
        //
        ...
        public string SyncImages(String ClientRequestId ,string tag = "durban" ,string keyword = "landmark")
        {
            //
            ...
            string _response = readAPI("https://api.flickr.com/services/rest/?method=flickr.photos.search&tags=" + tag + "&text=" + keyword + "&safe_search=&format=rest&api_key=12ba11510a21ba897117735df462cdca&extras=description, date_upload, date_taken, owner_name, tags, description", tag, keyword, ClientRequestId);
            return _response;
        }
        //
        ...
        public void UpdateStatus(String ClientRequestID, String Status)
        {
            ...
            {
                RunSQL("EXEC UpdateStatus '" + ClientRequestID + "','" + Status + "'");
            }
            else {
                ...
                scnstat.ClientRequestId = ClientRequestID;
                scnstat.ClientRequestIdStatus = Status;

                string imgInfo = Newtonsoft.Json.JsonConvert.SerializeObject(scnstat);
                PostData<object> postingData = imgInfo;
                dynamic result0 = client.DoRequest<object>(HttpMethod.PUT, scnstat.ClientRequestId, postingData);
            }
        ...
        public string readAPI(string apiURL, String tag,string keyword,string ClientRequestID )
        {
           ...
           // RunSQL("EXEC UpdateStatus '" + ClientRequestID + "','Connecting'");
            UpdateStatus(ClientRequestID, "Connecting");
            try
            {
            ...
            {
                UpdateStatus(ClientRequestID, "Connect to API error");
                return "'Error' : 'Connect to API error'";
            }
            UpdateStatus(ClientRequestID, "Processing data stream");
            int i = 0;
            ...
            //_CompletionStatus = _Row / ((System.Xml.NameTable)(xmlreader.)).count;
            while (xmlreader.Read())
            {
              //  UpdateStatus(ClientRequestID, XmlReaderExtensions.GetPosition(xmlreader, reader) + "%");
                switch (xmlreader.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        {
                            i++;
                            UpdateStatus(ClientRequestID, "Found Image " + i);
                            ...
                            {
                                UpdateStatus(ClientRequestID, "Adding Image " + i);
            ...
            UpdateStatus(ClientRequestID, "Completed " + tag);
            return "";
        }

        private void UpdateStatus(string clientRequestID, long v)
        {
        ...
        //
        public string SaveImage(image img)
        {
            if (System.Configuration.ConfigurationManager.AppSettings["StorageType"].Equals("SQL"))
            {
                string strSQL = "IF (SELECT count(*) FROM imagelib with (nolock) where sourceURL='" + img.sourceURL + "')=0  INSERT INTO imagelib(imgGUID,keyword,localURL,source,sourceURL,tag,title,dateHit,description,tags,owner,origin,tagKey,dateTaken,sourceServer,sourceID,sourceSecret,sourceFarm,webImageBase64Encoded) VALUES(";
                strSQL += "'" + img.imgGUID + "',";
                strSQL += "'" + img.keyword + "',";
                strSQL += "'" + img.localURL + "',";
                strSQL += "'" + img.source + "',";
                ...
                strSQL += "'" + img.webImageBase64Encoded + "') ELSE UPDATE imagelib SET ";
                strSQL += "keyword ='" + img.keyword + "',";
                strSQL += "localurl ='" + img.localURL + "',";
                strSQL += "source ='" + img.source + "',";
                strSQL += "tag ='" + img.tag + "',";
                strSQL += "title ='" + img.title + "',";
                strSQL += "dateHit ='" + img.dateHit + "',";
                strSQL += "description ='" + img.description + "',";
                strSQL += "tags ='" + img.tags + "',";
                strSQL += "owner ='" + img.owner + "',";
                strSQL += "origin ='" + img.keyword + "',";
                strSQL += "tagKey ='" + img.tagKey + "',";
                strSQL += "dateTaken ='" + img.keyword + "',";
                strSQL += "sourceServer ='" + img.sourceServer + "',";
            ...
            else
            {
                ...
                var query = client.Search<image>(s => s
                          ...
                          .Filter(fl => fl
                                .Term(t => t
                                .Field(f => f.tagKey)
                                .Value(searchtag.Replace(" ","").ToLower())
                ...
                var docs = query.Documents;
                foreach (image imgresult in docs)
                {
                    try
                    {
                        img.imgGUID =imgresult.imgGUID;
                        img.title = imgresult.title;
                        img.tag = imgresult.tag;
                        img.source = imgresult.source;
                        img.localURL = imgresult.localURL;
                        img.sourceURL = imgresult.sourceURL;
                        img.keyword = imgresult.keyword;
                        img.origin = imgresult.origin;
                        img.owner = imgresult.owner;
                        img.tags = imgresult.tags;
                        img.dateHit = imgresult.dateHit;
                        img.dateTaken = imgresult.dateTaken;
                        img.description = imgresult.description;
                        img.webImageBase64Encoded = imgresult.webImageBase64Encoded;
                        imagelist.Add(img);
                    }
                    catch (Exception)
                    {
